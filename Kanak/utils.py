...
def save_parquet(df: pd.DataFrame, path: Path, **kwargs) -> None:
    ...
    logging.getLogger(__name__).info("Saved %s  (%.1f KB, %d rows)", path, size_kb, len(df))
        ...
        try:
            ...
            if resp.status_code == 403:
                ...
                logging.getLogger(__name__).warning(
                    "Rate limited. Sleeping %d s …", wait
                )
                time.sleep(wait)
                continue
            ...
            return {
                ...
                "has_readme":       True,  # presence of /readme endpoint indicates one
            }
        except requests.RequestException as exc:
            logging.getLogger(__name__).warning(
                "Attempt %d/%d for %s failed: %s", attempt + 1, retries, repo_full_name, exc
            )
...
def enrich_repos_with_github_api(
    ...
    for i, name in enumerate(repo_names[:max_repos]):
        ...
        if meta:
            meta["repo_name"] = name
            results.append(meta)
        time.sleep(delay_s)   # gentle throttle
